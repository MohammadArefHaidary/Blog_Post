...
<div class="absolute w-full left-0 top-0 p-6 flex justify-between items-center space-x-6">
    ...
    <div class=" flex-grow flex items-center">
          ...
          <span class="mr-1">/</span> <input type="text" class="p-0 border-none focus:ring-0 w-full" v-model="post.
          slug" spellcheck="false" v-on:click="$event.target.select()">
    ...
    <div class="flex items-center space-x-6">
          ...
          <button v-on:click="post.published = !post.published" class="text-sm font-medium" v-bind:class="{
            'text-pink-500': !post.published
          }">
         ...
         <router-link :to="{name: 'post' , params: {slug: post.slug}}" class="text-sm font-medium 
         text-gray-800">Preview</router-link>
...
<div>
    ...
    <ResizeTextarea v-if="post.title">
      ...
      <textarea :ref="el" v-model="post.title" v-on:input="resize" class="block text-center text-4xl sm:text-6xl leading-10 font-extrabold 
      tracking-tight text-gray-900 border-none focus:ring-0 resize-none p-0" rows="1"></textarea>
        </template>
    </ResizeTextarea> 
